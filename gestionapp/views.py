...
def adherent_detail(request, pk):
    ...
    return render(request, 'gestion/detail_adherents.html', {'adherent': adherent})
...
def adherent_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            adherent = form.save()
            return redirect('gestionapp:adherent_detail', pk=adherent.pk)
    else:
        form = AdherentForm()
    return render(request, 'gestion/adherent.html', {'form': form})
